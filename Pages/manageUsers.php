                    ...
                    <?php while ($row = $user_details_result->fetch_assoc()) : ?>
                        <div class="card mb-3 border-0">
                            <div class="card-body d-flex flex-column justify-content-center align-items-center position-relative">
                                <div class="bg-gradient shadow-lg rounded-circle mb-3" style="width: 100px; height: 100px; overflow: hidden;">
                                    <?php if (!empty($row['profile_image'])) : ?>
                                        <img src="data:image/jpeg;base64,<?= $row['profile_image'] ?>" alt="Profile Image" class="profile-pic img-fluid rounded-circle" style="width: 100%; height: 100%; object-fit: cover;">
                                    <?php else : ?>
                                        <div class="signature-bg-color shadow-lg rounded-circle text-white d-flex justify-content-center align-items-center" style="width: 100%; height: 100%;">
                                            <h3 class="p-0 m-0"><?= strtoupper(substr($row['first_name'], 0, 1) . substr($row['last_name'], 0, 1)) ?></h3>
                                        </div>
                                    <?php endif; ?>
                                </div>
                                ...
                                <div class="d-flex flex-wrap justify-content-center">
                                    <div class="col-md-4 mb-2 m-1">
                                        <a href="Pages/ProfilePage.php?employee_id=<?= $row['employee_id'] ?>" target="_blank" class="btn btn-dark w-100"><small>Profile <i class="fa-solid fa-up-right-from-square fa-sm"></i></small></a>
                                    </div>
                                    <div class="col-md-4 mb-2 m-1">
                                        <button class="btn text-white editUserModalBtn w-100" style="background-color: #043f9d" data-employee-id="<?= $row['employee_id'] ?>" data-username="<?= $row['username'] ?>" data-password="<?= $row['password'] ?>" data-first-name="<?= $row['first_name'] ?>" data-last-name="<?= $row['last_name'] ?>"> <small>Edit</small> <i class="fa-regular fa-pen-to-square fa-sm mx-1"></i></button>
                                    </div>
                                    <div class="col-md-4 mb-2 m-1">
                                        <button class="btn btn-danger w-100 deleteUserBtn" data-employee-id="<?= $row['employee_id'] ?>"> Delete </button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="table-responsive rounded-3 shadow-lg bg-light m-0">
                        <table class="table table-hover mb-0 pb-0">
                            ...
                            <tbody>
                                ...
                                <?php while ($row = $user_details_result->fetch_assoc()) : ?>
                                    <tr class="text-center">
                                        <td class="py-4 align-middle"><?= $row['first_name'] . ' ' . $row['last_name'] ?></td>
                                        <td class="py-4 align-middle"><?= $row['employee_id'] ?></td>
                                        <td class="py-4 align-middle">
                                            <button class="btn">
                                                <a href="Pages/ProfilePage.php?employee_id=<?= $row['employee_id'] ?>" target="_blank"><i class="fa-solid fa-up-right-from-square fa-sm m-1 text-dark"></i></a>
                                            </button>
                                            <button class="btn editUserModalBtn" data-bs-toggle="modal" data-bs-target="#editUserModal" data-employee-id="<?= $row['employee_id'] ?>" data-username="<?= $row['username'] ?>" data-password="<?= $row['password'] ?>" data-first-name="<?= $row['first_name'] ?>" data-last-name="<?= $row['last_name'] ?>">
                                                <i class="fa-regular fa-pen-to-square signature-color m-1"></i>
                                            </button>

                                            <button class="deleteUserBtn btn" data-bs-toggle="modal" data-bs-target="#deleteConfirmationModal" data-employee-id="<?= $row['employee_id'] ?>">
            ...
            <div class="col-lg-2 order-1 order-lg-2">
                <div class="d-none d-lg-block">
                    <div class="bg-light d-flex justify-content-center flex-column align-items-center rounded-3 p-4 shadow-lg">
                        <button class="btn signature-btn p-3 col-10 d-flex flex-column justify-content-center align-items-center col-6 col-lg-12" id="addUserModalBtn" data-bs-toggle="modal" data-bs-target="#addUserModal"><i class="fa-solid fa-user-plus fa-3x signature-color"></i><span class="mt-2 signature-color fw-bold"> Add New User </span></button>
                ...
                <div class="col-12 d-lg-none bg-white rounded-3 p-4 mb-4 shadow-lg">
                    <div class="row">
                        <div class="col-12 col-md-4 mb-3 mb-md-0">
                            <button class="btn signature-btn p-3 w-100" id="addUserModalBtnMobile" data-bs-toggle="modal" data-bs-target="#addUserModal">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="POST">
                    ...
                    <div class="modal-body">
                        <h4 class="mt-4 mb-4 fw-bold signature-color text-center signature-bg-color text-white py-2 rounded-3">
