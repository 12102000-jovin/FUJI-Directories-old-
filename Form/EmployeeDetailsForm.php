...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sql = "INSERT INTO employees (first_name, last_name, gender, dob, visa, visa_expiry_date , address, email, phone_number, emergency_contact_phone_number, emergency_contact_name, emergency_contact_relationship, employee_id, start_date, employment_type, department, section, position, pay_rate, bank_building_society, bsb, account_number, superannuation_fund_name, unique_superannuation_identifier, superannuation_member_number, tax_file_number, higher_education_loan_programme, financial_supplement_debt, profile_image) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssssssssssssssisssssssiis", $firstName, $lastName, $gender, $dob, $visaStatus, $visaExpiryDate, $address, $email, $phoneNumber, $emergencyContact, $emergencyContactName, $emergencyContactRelationship, $employeeId, $startDate, $employmentType, $department, $section, $position, $payRate, $bankBuildingSociety, $bsb, $accountNumber, $superannuationFundName, $uniqueSuperannuatioIdentifier, $superannuationMemberNumber, $taxFileNumber, $higherEducationLoanProgramme, $financialSupplementDebt, $encodedImage);
...
<div class="row-cols-1 row-cols-md-3">
            ...
            <div class="row">
                ...
                <div class="col-12 col-lg-6 px-md-5">
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-4 mt-3" id="sectionField" style="display: none;">
    ...
    <script>
        ...
        departmentSelect.addEventListener('change', function () {
            ...
            // Check if selected department is 'Sheet Metal'
            if (selectedDepartment === 'Sheet Metal') {
                // Show the section field
                sectionField.style.display = 'block';
                // Show only the 'Sheet Metal' option
                sectionSelect.innerHTML = '<option disabled selected hidden>Choose Section</option><option value="Painter" id="painterSection">Painter</option>';
            } else if (selectedDepartment === 'Electrical') {
                // Show the section field
                sectionField.style.display = 'block';
                // Show only the 'Electrical' option
                sectionSelect.innerHTML = '<option disabled selected hidden>Choose Section</option><option value="Panel" id="panelSection">Panel</option><option value="Roof" id="roofSection">Roof</option>';
            } else if (selectedDepartment === 'Office') {
                // Show the section field
                sectionField.style.display = 'block';
                // Show only the 'Office' option
                sectionSelect.innerHTML = '<option disabled selected hidden>Choose Section</option><option value="Engineering" id="engineerSection">Engineering</option>';
            } else {
                // Hide the section field for other departments
                sectionField.style.display = 'none';
                // Reset the section select
                sectionSelect.innerHTML = '<option disabled selected hidden>Choose Section</option>';
            }
        });
    </script>
</div>